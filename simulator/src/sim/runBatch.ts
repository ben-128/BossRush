...
function buildPolicies(opts: BatchOptions): Policy[] {
...
export function runBatch(
  ...
  const policies = buildPolicies(opts);
  ...
  for (let i = 0; i < opts.runs; i++) {
    ...
    const heroIds =
      opts.heroes === 'random'
        ? pickRng.shuffle(data.heroes.map((h) => h.id)).slice(0, opts.players)
        : opts.heroes.split(',').map((x) => x.trim()).slice(0, opts.players);
    ...
    if (onProgress && (i + 1) % Math.max(1, Math.floor(opts.runs / 20)) === 0) {
      onProgress(i + 1, opts.runs);
    }
  }
  ...
  const stats: BatchStats = {
    ...
    wins,
    defeats,
    winrate: wins / Math.max(1, opts.runs),
    meanTurns: sumTurns / Math.max(1, opts.runs),
    meanTurnsVictory:

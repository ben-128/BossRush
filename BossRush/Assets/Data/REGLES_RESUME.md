...
| **Monstre** | Ennemi mineur. Animal corrompu par le Colosse, invoqué pendant le combat. Forme une file entre vous et le Colosse. |
...
| **Carte Chasse — Action** | Carte jouée depuis votre main. Attaque (mélée ou distance) ou effet immédiat (soin, pioche…). Défaussée après utilisation. |
...
| **Distance** | Attaque qui cible n'importe quel monstre de la file ou directement le Colosse. |
...
| Fiches Boss | 10 | 1 par boss (stats + passif) |
...
2. **Fiche Boss** : placer la fiche du boss choisi à côté de sa carte. Elle indique ses stats (dégâts, nb invocations, action spéciale, passif).
...
- **Attaque** (mélée ou distance) → placer sous la cible pour matérialiser les dégâts.
...
|---|---|---|
| Guerrier | Armure | Résistant, mélée, soigne/prévient les dégâts |
| Mage | Élémentaire | Gros dégâts (distance et mélée) |
...
À tout moment pendant son tour, le héros peut activer sa capacité spéciale (1×/combat).

### 3. Carte Menace
Piocher 1 carte du deck Menace et la résoudre :
- **Assaut** : le colosse attaque (dégâts, invocation de monstres, soin du colosse…). Appliquer l'effet.
- **Événement** : une situation survient pendant le combat. Le héros actif choisit entre les options proposées.

### 4. Monstres
S'il y a un monstre en tête de file, il attaque le héros actif.

### 5. Tour suivant

Continuer jusqu'à victoire ou défaite. Si le deck Menace est épuisé, mélanger la défausse Menace pour reformer la pile.
...
### File de monstres
- Nouveaux monstres → **fond** de file (côté boss).
- **Mélée** → cible uniquement la tête de file. Si vide, frappe le boss.
- **Distance** → cible librement n'importe quel monstre ou le boss.
...
- **Vie** : dégâts max avant hors combat.
...
### Fiche Boss (stats)
Chaque boss possède une fiche avec 4 stats consultées lors des épreuves :
| Stat | Utilisation |
|---|---|
| **Dégâts** | Valeur infligée par les cartes Assaut marquées `?` |
| **Nb Invocations** | Nombre de monstres piochés par les cartes Invocation |
...
- **Assaut** : attaque (`?` dégâts → voir fiche boss), invocation (`?` monstres → voir fiche boss), action spéciale (→ voir fiche boss), soin du boss, ordre d'attaque monstre
...
├─────────────────────────────────────────┤
│ 2. CAPACITÉ SPÉCIALE (optionnel, 1x)    │
├─────────────────────────────────────────┤
│ 3. CARTE ÉPREUVE                        │
...
- **Débordement** : un effet demande de piocher un monstre et la pile Monstres est vide. Les héros sont submergés par les créatures — défaite immédiate.
